 ...
 *
 *   This file is part of SimSens software.
 *
 *   SimSens is free software: you can redistribute it and/or modify
 *   it under the terms of the GNU General Public License as published by
 *   the Free Software Foundation, either version 3 of the License, or
 *   (at your option) any later version.
 *
 *   SimSens is distributed in the hope that it will be useful,
...
namespace evolve{    
    ...
    //functions to rotate around vector
    __hydra_dual__
    inline void _getdiffvect(double vecdir[],double startvec[], double outvec[], double angle, double modulus){
        double cosphi = cos(angle);
        double sinphi = sin(angle);
        double onemcosphi = 1 - cosphi;
        
        //define rotation matric around a vector
        double R11 = cosphi+vecdir[0]*vecdir[0]*onemcosphi;
        double R21 = vecdir[0]*vecdir[1]*onemcosphi+vecdir[2]*sinphi;
        double R31 = vecdir[0]*vecdir[2]*onemcosphi-vecdir[1]*sinphi;
        
        double R12 = vecdir[0]*vecdir[1]*onemcosphi-vecdir[2]*sinphi;
        double R22 = cosphi+vecdir[1]*vecdir[1]*onemcosphi;
        ...
        double R33 = cosphi+vecdir[2]*vecdir[2]*onemcosphi;
        
        outvec[0] = (startvec[0]*R11+startvec[1]*R12+startvec[2]*R13)*modulus;
        outvec[1] = (startvec[0]*R21+startvec[1]*R22+startvec[2]*R23)*modulus;
        outvec[0] = (startvec[0]*R31+startvec[1]*R32+startvec[2]*R33)*modulus;
    }
    
    __hydra_dual__
    inline void getdiffvect(double vecdir[], double outvec[], double angle, double modulus){
        double _vecdir[3];
        double modvecdir = sqrt(vecdir[0]*vecdir[0]+vecdir[1]*vecdir[1]+vecdir[2]*vecdir[2]);
        _vecdir[0] = vecdir[0]/modvecdir;
        _vecdir[1] = vecdir[1]/modvecdir;
        _vecdir[2] = vecdir[2]/modvecdir;
        double startvec[3];
        startvec[2] = 0.;
        startvec[1] = 1.;
        startvec[0] = -vecdir[1]/vecdir[0]*startvec[1];
        double startvecmod = sqrt(startvec[0]*startvec[0]+startvec[1]*startvec[1]+startvec[2]*startvec[2]);
        startvec[0] = startvec[0]/startvecmod;
        startvec[1] = startvec[1]/startvecmod;
        startvec[2] = startvec[2]/startvecmod;
        
        _getdiffvect(_vecdir, startvec, outvec, angle, modulus);
    }
...
//             hydra::get<_tc_gauss_z>(p) = az/fStep;
    ...
    struct ApplyRamo
    {
        ...
        void operator()(Particle p){
            ...
            //set diff velocity vector
            double veldiff[3]={0,0,0};
            double velpoint[3]={k1x,k1y,k1z};
            getdiffvect(velpoint, veldiff, hydra::get<_tc_angle_1>(p), hydra::get<_tc_gauss_x>(p));
            
            hydra::get<_tc_gauss_x>(p) = veldiff[0];
            hydra::get<_tc_gauss_y>(p) = veldiff[1];
            hydra::get<_tc_gauss_z>(p) = veldiff[2];
